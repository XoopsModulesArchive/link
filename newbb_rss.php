...
$filename = 'cache/xhnewbb.rss'; //File to read/write
...
if ($fd and (time() - filemtime($filename) < $timespan)) {
    ...
    if (!$result) {
        ...
        $temp .= ' <link>' . XOOPS_URL . "/modules/xhnewbb/</link>\n";
        ...
        while (false !== ($myrow = $xoopsDB->fetchArray($result))) {
            // $myrow = str_replace("?, "&eacute;", $myrow);
            ...
            $temp .= ' <link>' . XOOPS_URL . '/modules/xhnewbb/viewtopic.php?topic_id=' . htmlspecialchars($myrow['topic_id'], ENT_QUOTES | ENT_HTML5) . '&amp;forum=' . htmlspecialchars($myrow['forum_id'], ENT_QUOTES | ENT_HTML5) . "</link>\n";
