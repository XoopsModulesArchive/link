...
$filename = 'cache/mylinks.rss'; //File to read/write
...
if ($fd and (time() - filemtime($filename) < $timespan)) {
    ...
    if (!$result) {
        ...
        $temp .= " <channel>\n";
        $temp .= ' <title>' . $xoopsConfig['sitename'] . " - Links </title>\n";
        $temp .= ' <link>' . XOOPS_URL . "/modules/mylinks</link>\n";
...
$temp .= " < link > '.XOOPS_URL.' / modules / mylinks / index . php ? lid = ".htmlspecialchars($myrow['lid'])." < / link > \n";
$temp .= " < / item > \n";
}
$temp .= " < / channel > \n";
$temp .= " < / rss > "; 
